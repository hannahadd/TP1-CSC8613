...
payments_agg_90d : conserve les agrégats de paiement sur 90 jours, (par exemple le nombre d’échecs de prélèvement)
support_agg_90d : contient les interactions avec le support client (tickets, temps moyen de résolution) sur 90 jours
labels : correspond à la table cible du modèle de churn, il indique si un utilisateur a résilié (churn_label)
...
b) La fonction upsert_csv lit un fichier CSV avec pandas, fait quelques conversions de type, puis charge les données dans une table temporaire dans PostgreSQL. Ensuite, elle insère les données de cette table temporaire vers la table cible avec une clause ON CONFLICT sur la clé primaire : si une ligne existe déjà, elle est mise à jour, sinon elle est insérée. On obtient ainsi un “upsert” idempotent : relancer le flow sur les mêmes fichiers ne crée pas de doublons, mais met simplement les données à jour.
...
## Exercice 4:
a) La fonction validate_with_ge ajoute une étape de contrôle qualité dans le pipeline. Elle lit une table depuis PostgreSQL, applique des règles (expectations) sur la structure et sur la cohérence des valeurs. Dans le cas où une règle échoue, une exception est levée et le flow Prefect s’arrête, ce qui évite de propager des données incorrectes vers les étapes suivantes (snapshots / entraînement).
...
Les tables live représentent l’état courant des données, susceptible d’évoluer à tout moment (nouveaux utilisateurs, mises à jour, corrections). Entraîner un modèle directement sur ces tables pose des problèmes :
d’une part, les données peuvent changer entre deux entraînements, ce qui rend les résultats non reproductibles, aussi cela introduit un risque de data leakage, car le modèle pourrait indirectement exploiter des informations futures par rapport à la période qu’il est censé prédire.
...
Une difficulté rencontrée durant ce TP a été l’absence de snapshots pour le mois month_001 lors de la première exécution du pipeline. Le flow Prefect s’exécute sans erreur, mais aucune donnée n’était visible pour la date as_of = 2024-02-29. J'ai corrigé ce problème en vérifiant les variables d’environnement passées au conteneur (SEED_DIR et AS_OF) ainsi que le montage correct des volumes Docker, et ensuite en relançant le flow avec les bons paramètres. Cela a permis de créer correctement les snapshots pour les deux périodes.
