...
# Dates par défaut (month_000 vs month_001) : on les garde
...
def run_evidently(reference_df: pd.DataFrame, current_df: pd.DataFrame, as_of_ref: str, as_of_cur: str):
    ...
    # Seuil Evidently (dataset drift interne) : arbitraire pour le TP
...
def decide_action(as_of_ref: str, as_of_cur: str, drift_share: float, target_drift: float, threshold: float = 0.02) -> str:
    """
    Si drift_share >= threshold -> on déclenche VRAIMENT train_and_compare_flow(as_of=as_of_cur).
    """
    if drift_share >= threshold:
        decision = train_and_compare_flow(as_of=as_of_cur)
        return (
            f"RETRAINING_TRIGGERED drift_share={drift_share:.2f} >= {threshold:.2f} -> {decision} "
...
def monitor_month_flow(
    ...
    threshold: float = 0.02,  # seuil retraining par défaut demandé
):
...
if __name__ == "__main__":
    # Exécution normale (month_000 vs month_001) avec threshold=0.02
    monitor_month_flow()

    # Si ton drift ne déclenche pas, décommente pour une démo forcée :
