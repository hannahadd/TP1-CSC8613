...
# IMPORTANT: dans Docker, MLflow est joignable via le hostname "mlflow" + port 5000
...
def fetch_labels(engine, as_of):
    # essaie d'abord un schéma "user_id, period_start, churn_label" si jamais le prof l'a prévu
    try:
        q = """
    ...
    # sinon: ton schéma actuel (user_id, churn_label)
...
# --------------------
def main():
    # TODO 1: configurer MLflow (tracking URI + experiment)
    ...
    features = [
        ...
        # payments_agg_90d_fv (1)
        "payments_agg_90d_fv:failed_payments_90d",

        # support_agg_90d_fv (2)
    ...
    # TODO 3: merge features + labels sur (user_id, event_timestamp)
    ...
    # TODO 7: run MLflow + fit + metrics + logs
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        ...
        mlflow.log_metric("auc", float(auc) if auc == auc else np.nan)  # handle nan
        mlflow.log_metric("f1", float(f1))
        mlflow.log_metric("acc", float(acc))
        mlflow.log_metric("train_time_s", float(train_time))

        # artefact JSON de schema de features
        mlflow.log_dict(
            {"categorical_cols": cat_cols, "numeric_cols": num_cols},
            "feature_schema.json"
        )

        # TODO 8: signature + log_model Registry
        input_schema = Schema(
            [
        ...
        # TODO 9: on enregistre la PIPELINE (prep + modèle), pas juste clf
        ...
        print(f"[OK] Trained baseline RF. AUC={auc:.4f} F1={f1:.4f} ACC={acc:.4f} "
